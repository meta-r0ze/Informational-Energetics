...
Unlike open systems (e.g., biological cells, economies), the vacuum has no external environment. It cannot offload entropy, import energy, or borrow structural memory. As the totality of existence, there is no ``outside'' to provide initial conditions.

Informational Energetics therefore imposes the \textbf{Closure Constraint}: \textit{The substrate of reality must be entirely self-defining. It admits no external configuration memory, auxiliary structures, or free parameters. This is a boundary condition, not a dynamical principle: there is no ``outside'' to the vacuum, hence no auxiliary structure to hold selection information. Any candidate substrate requiring outside data to specify its geometry is not a fundamental ground state; it presupposes one.}

This constraint acts as a primary filter. A valid substrate cannot borrow its geometry; its structure must be the inescapable, deterministic consequence of satisfying its own architectural requirements.
...
To minimize Algorithmic Complexity, the substrate must be \textbf{Homogeneous}. In Algorithmic Information Theory, the structural complexity of a random graph scales with its size ($K(S) \propto N$). For the universe to be scalable without additional processing overhead, its Kolmogorov Complexity must be $O(1)$, independent of size (Closure Constraint). This requires that the local topology be the same regardless of location and direction, minimizing the structural description to a single, repeating rule.

\begin{itemize}
    \item \textbf{The Requirement:} The universe must be a \textbf{Lattice}, a structure that is both discrete (finite) and has a constant structure in every direction (regular) rather than a continuous manifold or a random graph.
\end{itemize}

\subsection{Persistence Requires \texorpdfstring{$MAP, PRO$}{MAPPRO} via Unitarity}
\label{sec:unitarity}
...
To prevent information loss via destructive interference or Landauer erasure, the encoding operation (write) and the decoding operation (read) must be informationally equivalent. This is enforced by the modular transformation $S: \tau \to -1/\tau$, which maps the lattice to its reciprocal (Fourier dual).
...
\begin{itemize}
    ...
    \item \textbf{The Requirement:} To express its full informational capacity, the system must serialize all $N$ potential states into $\Delta$ discrete temporal slots per fundamental cycle. By the Pigeonhole Principle, if $\Delta \le N$, the number of states meets or exceeds the available temporal slots. Consequently, at least one temporal coordinate must simultaneously host multiple distinct states. This results in \textbf{Causal Aliasing}: distinct information states become concurrently superimposed in time, destroying the system's ability to maintain a well-defined serial history.
    
    \item \textbf{Requirement:}
    To enforce a strict arrow of time and satisfy the Causality requirement, the projection must satisfy the Persistence Inequality: $\Delta > N$. The temporal container must strictly exceed the information content it holds.
\end{itemize}
