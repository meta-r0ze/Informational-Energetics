...
\subsubsection{Lattice (Discrete + Homogeneous)} A structure that is both discrete (finite information density) and homogeneous (constant structure in every direction) is a \textbf{Lattice}. This excludes continuous manifolds (infinite description) and random graphs (non-constant complexity). The lattice provides a discrete address space; it does not require the information payload residing at those addresses to be discrete. The Map and Protocol pillars may encode continuous state vectors within each node, subject only to the finiteness and unitarity constraints derived next.
