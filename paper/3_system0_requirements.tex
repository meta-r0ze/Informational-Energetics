...
As the minimal persistent substrate from which all observable structures emerge, no deeper system exists beneath it. It cannot obtain configuration information, boundary conditions, synchronization signals, or memory from an external source. We formalize this absolute isolation as:
\label{subsec:closure}
\begin{quote}
\textbf{The Closure Constraint (Zero-Flux Baseline):} The substrate of reality must be entirely self-defining. Its geometry must arise exclusively from its own architectural requirements.
\end{quote}

Any candidate substrate requiring external parameters violates this baseline and is not a fundamental ground state. By governed the vacuum by this constraint, we create the most rigorously constrained test case possible. If the six requirements of persistence alone cannot uniquely derive the vacuum's mathematical structure, the IE framework is not universal. We refer to this specific mapping of IE onto the vacuum substrate as the 
\textbf{$E_8$-Persistence Theory} ($E_8$P), distinguishing the universal 
framework from the validated system.

We explicitly restrict the bounds of this paper to deriving the static geometric baseline, providing the foundation for constructing the continuous dynamic Effective Field Theory (EFT) Lagrangian in future work.
