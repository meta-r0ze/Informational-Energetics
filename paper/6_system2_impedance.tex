...
On a homogeneous network, any localized state naturally tends to disperse into geometric noise. To remain persistently observable, a configuration must maintain a stable topological partition across its fundamental causal cycle. This requires satisfying all six orthogonal pillars of the 
persistence architecture.

Because these pillars operate on independent configuration subspaces with vanishing mutual information (\cref{sec:entropic_balance_sheet}), their partition functions factorize multiplicatively ($\mathcal{Z}_{\mathrm{total}} = \prod_i \mathcal{Z}_i$). Because $S_\Phi = -\ln \mathcal{Z}_{\mathrm{total}}$, the independent entropic costs combine strictly additively:
\begin{equation}
    Z_{\mathrm{geo}} = \sum_{i=1}^6 Z_i .
\end{equation}

To calculate this total impedance, we classify the six architectural components into \emph{Direct Structural Costs} ($Z_i > 0$), which represent the raw entropic overhead of preventing failure, and \emph{Organizational Gains} ($Z_i < 0$), which actively suppress failure rates, reducing the net system impedance.
...
Capacity contributes a \textbf{Direct Structural Cost} that prevents \emph{instantaneous delocalization}. It is the absolute geometric allocation of the spatial envelope: the continuous perimeter required to enclose the configuration across its fundamental cycle. Without this base structural cost, the state would serialize openly at the bandwidth limit ($c=1$) and dissolve into the bulk.
...
Map contributes a \textbf{Direct Structural Cost} that prevents \emph{boundary dissolution}. It is the absolute allocation of a topological boundary ($\chi = 2$) that prevents internal state data from leaking into the unquantized background.
...
Governor contributes an \textbf{Organizational Gain} that prevents \emph{feedback runaway}. It enforces the topological boundary as a stabilizing constraint distributed uniformly across the temporal loop. Its contribution is therefore the linear density of the boundary invariant to the loop length.
...
Margin contributes a \textbf{Direct Structural Cost} that prevents \emph{stochastic fragility}. It is the resolution threshold that buffers the signal against geometric noise. Its contribution is therefore a fractional dilution factor: one quantum of structured information divided by the minimal configuration volume.
...
\vspace{0.5em}
\noindent\textit{Proof.} The theorem rests on four structural deductions forced by the Closure Constraint and the Quiescent Equilibrium baseline:
...
To combine these pillars without cross-terms ($Z_i Z_j$), the mutual information between them must vanish identically ($I(A;B)=0$). As established in the Entropic Balance Sheet (\cref{sec:entropic_balance_sheet}), mutual information between orthogonal domains vanishes at zero flux, yielding a factorized partition function ($\mathcal{Z}_{\mathrm{total}} = \prod_i \mathcal{Z}_i$). The resulting entropic action $S_\Phi \propto -\ln \mathcal{Z}_{\mathrm{total}} = \sum_i S_i$ is strictly additive. Defining total geometric impedance as this action per unit flux quantum ($Z_{\mathrm{geo}} \equiv S_\Phi / \Phi_{\mathrm{top}}$) maps this directly to $Z_{\mathrm{geo}} = \sum Z_i$, ensuring all higher-order cross-terms vanish.

\subsubsection{Sign Assignment via the Entropic Balance Sheet}
The assignment of the signs $s_i \in \{+1, -1\}$ is uniquely determined by the Entropic Balance Sheet (\cref{sec:entropic_balance_sheet}). Direct physical resource allocations and bounding constraints (Capacity, Map, Toll, Margin) strictly increase the total entropic action ($s_i = +1$). Conversely, coordination mechanisms (Protocol, Governor) that actively suppress uncoordinated dissipation enter as net action reductions ($s_i = -1$). 
