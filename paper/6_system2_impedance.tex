...
On a homogeneous network, any localized state naturally tends to disperse into geometric noise. To remain persistently observable, a configuration must maintain a stable topological partition across its fundamental causal cycle.  By the Entropic Balance Sheet (\Cref{sec:entropic_balance_sheet}), the six orthogonal pillars contribute either Direct Structural Costs or Organizational Gains, which sum to the total geometric impedance.
...
The assignment of the signs $s_i \in \{+1, -1\}$ is uniquely determined by the Entropic Balance Sheet. Direct physical resource allocations and bounding constraints (Capacity, Map, Toll, Margin) strictly increase the total entropic action ($s_i = +1$). Conversely, coordination mechanisms (Protocol, Governor) that actively suppress uncoordinated dissipation enter as net action reductions ($s_i = -1$). 
