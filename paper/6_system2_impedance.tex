\section{System 2: The Geometric Initialization of Physics}
\label{sec:System2_initialization}

The $E_8$ lattice of System 1 is an 8-dimensional Euclidean crystal, beautiful, rigid, and causally inert. To become the universe, it must project onto a 4-dimensional Lorentzian manifold capable of supporting time evolution and information propagation.

This projection is not an assumption; it is the geometric consequence of the Causality constraint ($\Delta > N$). The temporal modulus $\Delta = 43$ requires a directed update sequence, which in turn requires a metric signature distinguishing ``before'' from ``after.'' We now derive the first physical constants as geometric invariants of this projection which future works will show that all other constants depend upon.

\section{System 2: The Geometric Impedance (\texorpdfstring{$\alpha^{-1}$}{alpha\string^-1})} \label{sec:geometric_impedance}
...
\end{equation}
This acts as the Ultraviolet Cutoff (Governor), preventing the field energy from diverging at small scales.
...
\begin{equation}
    Z_{MAR} = \frac{1}{H_{full} \cdot (\sigma + 1) \cdot \Delta^2} \approx +2.91 \times 10^{-6}
\end{equation}
