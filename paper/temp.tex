...
Capacity contributes the fundamental geometric impedance required to prevent a localized state from dissipating into the unconstrained Euclidean background (Information Starvation). This entropic footprint dominates the vacuum impedance ($\approx 99.9\%$). Without this cost, the state would propagate openly at the bandwidth limit ($c=1$) and delocalize completely.
...
Toll contributes the transition impedance required to select a valid, non-degenerate interaction from the unconstrained phase space, preventing Zeno Collapse. Geometrically, it is the fractional cost of executing a state transition; without it, causal history collapses into instantaneous updates.
...
In Quiescent Equilibrium, the six pillars operate on orthogonal, independent domains (\Cref{sec:entropic_balance_sheet}). At strict zero flux, no propagating excitations exist to mediate interactions between these domains; mutual information vanishes identically, forbidding cross-terms. This motivates the total geometric index $Z_{\text{geo}}$ as the linear combination of independent dimensionless contributions:

\begin{equation}
    Z_{\text{geo}} = \sum_{i=1}^6 Z_i.
\end{equation}

Each term $Z_i$ is a dimensionless quantity forced by the geometric and information-theoretic requirements of its pillar: boundary measure (CAP), topological invariant (MAP), coordination efficiency (PRO, GOV), transition rarity (TOL), and resolution dilution (MAR). All six are constructed from the same substrate invariants and operate on orthogonal domains. 

Two properties characterize this sum:
\begin{enumerate}
    \item \textbf{Completeness:} The functional form of each term is strictly forced by the geometric and information-theoretic requirements of its component. The six components exhaust the architectural degrees of freedom required for persistence. The derived geometric limits ($\Delta = 43$, $\nu = 16$, $\sigma = 5$, $\chi = 2$) exhaust the independent structural degrees of freedom of the projection. \textit{(Note that $\nu=16$ enters only through the total node capacity $N=2\nu=32$.)}
    
    \item \textbf{Signs:} The signs are fixed by the Entropic Balance Sheet (\Cref{sec:entropic_balance_sheet}). Structural overhead terms (CAP, MAP, TOL, MAR) are positive; coordination terms (PRO, GOV) reduce net dissipation and are negative.
\end{enumerate}

The geometric impedance is therefore given by expanding this sum into the individual geometric costs derived in \crefrange{subsec:alpha_cap}{subsec:alpha_mar}:
