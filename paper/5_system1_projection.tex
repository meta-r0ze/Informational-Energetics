...
\subsubsection{The Admissible Signatures}
...
\begin{enumerate}
    \item[\textbf{(R1)}] \emph{The Causal Torus.} As derived in \cref{sec:metric_signature}, the Lorentzian signature pairs the temporal step with the spatial step ($\delta t = i\,\delta x$). Causal propagation over a fundamental cycle $\Delta$ therefore forms a periodic $(1+1)$D loop. Topologically, this is a real torus $\mathbb{T}^2$; analytically, it is a complex torus $\mathbb{C}/\Lambda$ endowed with a complex structure that generates the imaginary quadratic field $K = \mathbb{Q}(\sqrt{-\Delta})$.
...
\begin{enumerate}
    \item[\textbf{(M1)}] \emph{Class Number (Solving R3).} If the causal loop can be rewired in multiple ways without changing its local appearance, the system cannot tell which history it followed, and the Map collapses. The number of distinct global rewirings that share identical local data is governed by the \textbf{class number} $h(D_K)$. If $h(D_K) > 1$, multiple non-isomorphic causal loops exist with the same local signature, destroying Unitarity by Causal Degeneracy. In the geometry of complex tori, each distinct rewiring corresponds to a non-principal ideal class; the total number of such classes is exactly $h(D_K)$. The uniqueness constraint therefore requires $h(D_K) = 1$.
...
% Establish the premises that force relational time: no external clock (Closure), no internal memory counter (Finiteness),  no surrogate clocks (Homogeneity).
By the Closure Constraint, the vacuum possesses no external reference clock. Time must therefore emerge relationally, defined exclusively by internal state transitions. By Finiteness, the node cannot allocate channel capacity to an internal memory counter to track elapsed empty intervals; by Homogeneity, spatial neighbors are geometrically identical during static intervals and cannot serve as surrogate clocks.

% Define the operational composition of a fundamental cycle: N active transitions that mark time, and M = Δ - N inert intervals that contain no temporal markers.
In a fundamental causal cycle of length $\Delta$, a node executes $N = 32$ active state transitions, leaving $M = \Delta - N$ inert intervals. An inert interval contains no state transition; by the definition of relational time, it contains no temporal marker.

% Identify the threat to Unitarity: consecutive inert intervals create temporal degeneracy because the system cannot distinguish their length, rendering the causal history non-injective.
A run of $k \ge 2$ consecutive inert intervals therefore contains exactly one temporal signature (the active transition at each end), but spans $k$ indistinguishable substeps. The temporal mapping is
non-injective: the system cannot reconstruct whether one, two, or $k$ empty steps occurred, violating the Unitarity requirement that causal history be uniquely invertible.

% State the structural requirement: to preserve injective causal history, consecutive inert intervals must be structurally forbidden.
For the causal history to remain uniquely reconstructable, consecutive inert intervals must be structurally forbidden ($k_{\max} = 1$).

% Derive the mathematical bound from the structural requirement using the pigeonhole principle on a periodic loop.
On a periodic loop, distributing $M$ inert intervals with no adjacent pairs requires at least as many active transitions as inert intervals to serve as separators. By the pigeonhole principle:
\begin{equation}
    N \ge M \implies N \ge (\Delta - N) \implies \Delta \le 2N = 64
\end{equation}
This bound is necessary and sufficient for unitary reconstructability under relational time.

% Results
With node capacity fixed at $N = 32$, this relational continuity limit restricts the fundamental cycle to $\Delta \le 64$. This eliminates the remaining Stark--Heegner candidates $67$ and $163$, isolating $\Delta = 43$ as the strictly unique geometric solution for the temporal period.
...
The vacuum's persistence depends on three hierarchical resolution limits, thresholds below which signals dissolve into the substrate. Operating on orthogonal channels, their description lengths combine additively.
...
The minimum information to specify a defect's internal state and local geometric footprint: the chiral channel configuration ($\nu=16$), the local interaction embedding ($\sigma=5$), and the boundary topological invariant ($\chi=2$). These orthogonal components set the internal resolution limit, the precision with which the vacuum distinguishes \emph{what} a defect is from \emph{where} it is.
...
\label{subsec:lembed}
The total information to specify a defect's complete configuration. Causality requires more than a static coordinate: the state must specify both its current manifold location and its causal successor
($D=4$ each), without external boundary conditions. This geometric doubling defines the minimal directed causal trajectory (the discrete analogue of kinematic phase space).
