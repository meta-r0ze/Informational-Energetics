...
\paragraph{Substrate load ($L_{\text{substrate}} = 188$)}  
...
No continuous degrees of freedom remain. The $E_8$ lattice projected onto a causal $D=4$ manifold is therefore a minimal persistent system: each architectural requirement is satisfied by a discrete invariant derived from the closure constraint. Together this establish the unique self-specifying substrate from which all observable structure must emerge.