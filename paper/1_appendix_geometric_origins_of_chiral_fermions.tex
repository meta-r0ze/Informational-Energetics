...
The $E_8$-Persistence theory employs the symmetric decomposition  $E_8 \to D_4 \oplus D_4$ (derived in \cref{sec:symmetric_decomposition}) coupled with the Metric Projection (derived in \cref{sec:metric_signature}), which falls outside this assumption. Here we provide the explicit technical demonstration that this metric signature change $(8,0) \to (1,3) \oplus (4,0)$ evades the no-go theorem at the \textbf{static quiescent-equilibrium baseline} by rendering mirror states geometrically inert from the perspective of causal evolution on the observable manifold. Dynamical suppression in the full effective field theory is left to future work.
