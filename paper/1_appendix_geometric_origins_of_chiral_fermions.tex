...
Distler and Garibaldi provided a rigorous service to the field by demonstrating that a continuous, algebraic embedding of the Standard Model within $E_8$ cannot accommodate chiral fermions without introducing unobserved ``mirror fermions.''
...
Our framework utilizes \textbf{Metric Projection} involving a signature change ($8,0 \to 1,3$). By explicitly changing the metric signature, the projector $P_L$ acts as a \textbf{Chiral Filter}.

As detailed in Section IV.C.2, the introduction of the time-like metric axis forces the decoupling of the ``Mirror'' sector to preserve norm positivity. The mirror fermions required by $E_8$ symmetry still exist in the mathematical structure of the lattice, but they are rendered geometrically orthogonal to the observer's timeline. They are not erased; they are causally disconnected.
