...
Distler and Garibaldi \cite{distler_there_2010} demonstrated that a signature-preserving embedding of the Standard Model within $E_8$ cannot accommodate chiral fermions without unobserved mirror partners. Their theorem holds for all embeddings where the algebraic structure remains Euclidean $(8,0)$. This appendix summarizes the derivation in Section \ref{sec:metric_signature} for readers seeking explicit verification that the metric signature change $(8,0) \to (1,3) \oplus (4,0)$ evades the theorem by rendering mirror states dynamically inert. No new results are introduced.

The $E_8$-Persistence theory employs the symmetric decomposition $E_8 \to D_4 \oplus D_4$ (derived in \cref{sec:symmetric_decomposition}) coupled with the Metric Projection (derived in \cref{sec:metric_signature}), which falls outside this assumption. 
