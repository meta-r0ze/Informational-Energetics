...
While open systems (like biology) persist by maximizing energy intake, a closed system at its ground state persists by minimizing entropic action, as there is no external energy source to offset dissipation.
...
Every system operates upon a \textbf{Substrate}: the physical medium imposing hard limits (bandwidth, latency, noise floor) within which the six pillars must function.
...
\begin{itemize}
    \item \textbf{Positive Terms (Costs):} Constraints that require energy to maintain or define (Capacity, Identity, Overhead, Margin). These increase the impedance.
    \item \textbf{Negative Terms (Gains):} Mechanisms that reduce dissipation through improved coordination. The Protocol (MI) corresponds to the Feedback path in the control loop. In standard negative feedback systems, this signal is \textit{subtracted} from the setpoint ($Error = Reference - Feedback$). Mathematically, this manifests as a negative term in the impedance summation, representing the information required to correct deviations. The Governor (G) prevents wasteful divergence. These represent \textbf{structural optimizations}, the difference between a disorganized system (high dissipation) and an organized one (low dissipation), not a violation of thermodynamics but a reduction in entropic burden.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{No Identity ($\Delta I \to 0$):} The \textit{Dissolution Mode}. The system lacks a reference signal (self-definition), causing the actuator to fire randomly and maximizing internal entropy. ($\tau \to 0$ via loss of boundary).
    ...
    \item \textbf{No Temporal Cost ($T \to 0$):} The \textit{Zeno Mode} (after Zeno's paradox of infinite subdivision). Instantaneous updates ($T \to 0$) result in a \textbf{Logical Singularity}. Information processing requires state transitions. A transition time of zero ($T \to 0$) implies infinite processing bandwidth; by the Shannon-Hartley theorem, this would require infinite signal power ($C \to \infty$), explicitly violating the \textbf{Finiteness} pillar. Physically, this is bounded by the Margolus-Levitin limit \cite{margolus_maximum_1998}, which establishes that the time $\Delta t$ required to transition between orthogonal states satisfies $\Delta t \ge \frac{h}{4\Delta E}$. A zero-cost transition collapses the distinction between cause and effect, rendering the state space non-computable ($\tau \to 0$ via \textbf{Causal Collapse}).
    \item \textbf{No Margin ($PM \to 0$):} The \textit{Fragility Mode}. Persistence requires a safety factor defined by $PM > k \cdot \sigma_{env}$, where $\sigma_{env}$ represents the variance of environmental fluctuations. A system at theoretical criticality ($PM \to 0$) survives only in a perfectly static environment. In any real-world substrate, the first non-zero fluctuation pushes the system state outside its basin of attraction, resulting in immediate failure ($\tau \to \text{Random Variable}$).
\end{itemize}
...
\textit{Note on Sufficiency:} Complex cognitive functions often cited in systems theory, such as \textbf{Memory} or \textbf{Prediction}, are not distinct pillars but emergent properties of this fundamental set. Memory is the persistence of the Identity ($\Delta I$) over time ($T$). Prediction is the operation of the Model ($\Delta I$) via the Protocol ($MI$). Thus, the six pillars constitute the irreducible basis set.
