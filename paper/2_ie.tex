...
The foundational premise of IE is \textbf{selectionist} rather than teleological: configurations that fail to persist leave no trace, while configurations that persist constitute the observable universe. This is not a statement about what systems ``want,'' but about what remains after entropic selection.

We formalize this as:

\begin{mdframed}[linewidth=1pt,linecolor=black,backgroundcolor=gray!10]
    The \textbf{Selection Principle}: Among all possible configurations, only those that minimize \textbf{Entropic Action} ($S_\Phi$) relative to structural complexity remain observable. The Persistence Value ($P$) quantifies this selection: $P \propto 1/S_\Phi$.
\end{mdframed}
...
where $\tau$ is the system's lifetime and $Z_{IE}(t)$ is the instantaneous Net Entropic Impedance, the time-dependent generalization of the static pillar sum in \cref{eq:entropic_balance}. For systems in 
\textbf{Quiescent Equilibrium}, where entropic drag is minimized to the theoretical floor and $Z_{IE}$ is constant, $S_\Phi$ approaches its minimum value $Z_{IE} \cdot \tau$, consistent with the system maintaining a stable configuration. \textbf{Note:} The full development of $S_\Phi$ as a variational principle for field-theoretic systems (including the derivation of the Lagrangian structure), is deferred to future work.
...
\begin{itemize}
    \item \textbf{Structural Costs ($+$):} The energetic overhead of existence. This includes maintaining boundaries (CAP, MAR), storing internal models (MAP), and the irreversible cost of state updates (TOL). These terms increase impedance.
...
\begin{itemize}
    ...
    \item \textbf{No Protocol ($PRO \to 0$):} The \textit{Decoherence Mode}. The CAP and MAP become statistically independent; commands are based on outdated states. ($\tau \to 0$ via Fragmentation).
...
Informational Energetics broadly describes the lifecycle of complex systems through phases of Genesis, Expansion, Stasis, and Collapse via the mathematics of logistic bifurcation. However, it is important to note that the system derived next (the Vacuum requirements, System 0) represents a specific thermodynamic state known as \textbf{Quiescent Equilibrium}.

In IE, a crucial distinction exists between a \textit{substrate} and the \textit{transient systems} it hosts. Hosted systems, whether biological organisms, economic markets, or the visible universe itself, progress through active lifecycles, consuming available capacity through expansion until eventual energetic collapse. 

A persistent substrate, however, cannot be in a state of expansion or brittle stasis. The fundamental laws of physics represent a system that has minimized its entropic drag to the absolute theoretical floor ($S_\Phi \to 0$). The geometric invariants derived in the following sections are not evolving parameters; they are the \textbf{Fixed Point Attractors} of the vacuum's self-optimization. The vacuum is not ``evolving'' new laws; it is persisting within the optimal solution, serving as the quiescent stage upon which all other systems play out their lifecycles.
...
IE makes a specific, falsifiable claim: that by restricting focus to persistent systems, we can derive exact structural constraints. This paper tests this claim in fundamental physics; extension to 
other domains remains future work.