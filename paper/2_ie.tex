...
The system exist ontop of \textbf{The Substrate:}: the medium that defines the hard limits (e.g., latency, bandwidth) within which the system must operate.
...
\begin{itemize}
    ...
    \item \textbf{Negative Terms (Gains):} Constraints that organize the flow, effectively increasing the conductance (or reducing the impedance) of the system (Protocol, Governor). In network theory, these act as \textit{Negative Resistance} elements, feeding order back into the loop to stabilize the flux.
\end{itemize}
...
While the field of complex adaptive systems has studied these dynamics for decades, Informational Energetics makes a specific, falsifiable claim: by restricting the focus to only those systems that successfully \textbf{persist}, we can move from qualitative description to a predictive, universal science.