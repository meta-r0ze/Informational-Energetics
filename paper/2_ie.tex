...
The fundamental imperative of persistence is to maximize the duration of existence against environmental entropy. We formalize this as follows:
...
While open systems (like biology) persist by maximizing energy intake, the vacuum is a closed system. Therefore, the persistence criterion shifts: rather than maximizing intake, the vacuum must \textbf{minimize information loss}.
    ...
    We define the \textbf{Dissipation Functional} ($\Phi_{\text{drag}}$) as the rate of irreversible information loss. According to Landauer's Principle, processing information requires energy, and erasing information generates heat (entropy). 

\begin{equation}
\Phi_{\text{drag}}[\psi] = \xi \cdot \mathbf{K}[\psi] \cdot \lambda_{\text{flux}}[\psi]
\end{equation}

Where:
\begin{itemize}
    \item $\mathbf{K}[\psi]$ (\textbf{Structural Complexity}): The information density of the state (Bits). In the physical limit, this corresponds to \textbf{Mass}.
    \item $\lambda_{\text{flux}}[\psi]$ (\textbf{Transition Flux}): The rate of state erasure or decoherence ($s^{-1}$). This corresponds to \textbf{Time} (Frequency).
    \item $\xi$ (\textbf{Landauer Coefficient}): The energetic cost of a bit transition ($\xi = k_B T_{\text{eff}} \ln 2$). This defines the thermal scale of the vacuum noise floor, ensuring finite action in the quantum limit.
\end{itemize}

To satisfy the Axiom, any persistent entity must implement a specific architecture comprising four structural pillars for information management, plus the thermodynamic overhead of operation.
...
The specific mapping of IE to a domain varies. Future papers will demonstrate examples such as the pillars mapping term-for-term to the Lagrangian of the Standard Model as well as other systems in physics.
...
\begin{description}
    ...
    \item[\textbf{Governor ($G$):}] \textbf{Stability Criterion (BIBO)} The negative feedback gain required to prevent unbounded divergence.
...
\begin{enumerate}
    ...
    \textit{Result:} $\tau \to 0$ due to thermodynamic equilibrium. (The Ghost).
    ...
    \textit{Result:} $\tau \to 0$ due to loss of boundary. (The Cloud).
    ...
    \textit{Result:} $\tau \to 0$ due to fragmentation. (The Schism).
    ...
    \textit{Result:} $\tau \to 0$ due to structural rupture. (The Explosion/Cancer).
...
Informational Energetics describes the lifecycle of systems through phases of Genesis, Expansion, Stasis, and Collapse. It is important to note that the system derived here, the Vacuum (System 0), represents a specific thermodynamic state known as \textbf{Quiescent Equilibrium}.
