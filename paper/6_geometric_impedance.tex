...
The Geometric Impedance sums contributions from distinct geometric sectors. This linear addition is justified by the \textbf{orthogonality} of the underlying geometric structures:

\textbf{1. Lattice Natural Units:} We adopt units where $\ell = c = \hbar = 1$. In this regime, action is a dimensionless scalar counting the number of fundamental quantum operations.

\textbf{2. Geometric Orthogonality:} The impedance sectors operate on disjoint geometric degrees of freedom within the projection:
\begin{itemize}
    \item \textbf{Metric Sector} ($\pi\Delta$): Depends on the temporal projection path (1-form).
    \item \textbf{Topological Sector} ($\chi$): Depends on the boundary closure (0-form).
    \item \textbf{Probabilistic Sector} ($N^{-3}$): Depends on the phase space volume (Measure).
\end{itemize}

Because these sectors couple to independent invariants of the $E_8 \to D_4$ projection, verified by the decomposition of the root system into orthogonal spacetime and internal symmetry sublattices there are no cross-terms (e.g., $\chi \cdot \Delta$) in the ground state action. The total impedance decomposes as the linear sum of the sector contributions:
\begin{equation}
    Z_{geo} = \sum Z_i
\end{equation}
...
\subsection{Derivation A: The Base Geometry (The Minimal Wilson Loop)}
...
\subsection{Derivation B: The Thermodynamic Corrections}
...
\textbf{Theorem:} Given a discrete $E_8$ lattice projected onto a causal $D=4$ manifold subject to the Persistence Principle, the Geometric Impedance $\alpha^{-1}$ is uniquely determined by the linear sum of the irreducible geometric sectors derived in Section C.

\textit{Proof:}
The Impedance Functional $Z[\Psi]$ must span all available degrees of freedom in the projection to maintain unitarity. As established in the System Specification (Section C), the projection geometry decomposes into exactly five irreducible sectors: Metric (1-Form), Topological (0-Form), Symmetry (Group), Conformal (Scale), and Entropic (Probabilistic).
