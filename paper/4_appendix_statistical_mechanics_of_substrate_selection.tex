...
\subsubsection*{A Note on Terminology: A Formal Analogy}

The formalism presented in this appendix employs the powerful mathematical machinery of statistical mechanics. It is essential to clarify that this is a \textbf{formal analogy} used to describe a selective process in an abstract configuration space, not a literal thermal system. The vacuum (System 0) is a zero-temperature quantum system.

The analogy is defined by the following mapping:
\begin{itemize}
    \item \textbf{The Ensemble:} Not a collection of particles in a heat bath, but the abstract ensemble of all possible geometric configurations ($\psi$) of the $E_8$ lattice.
    \item \textbf{The Cost Function:} The role of Energy ($E$) is replaced by our \textbf{Entropic Action ($S_{E_8}$)}, a dimensionless cost function that quantifies a configuration's violation of the fundamental principles of persistence (Unitarity, Causality, etc.).
    \item \textbf{The Selective Pressure:} The role of inverse temperature ($\beta = 1/k_B T$) is replaced by a ``Persistence Pressure'' ($\beta$). The limit $\beta \to \infty$ is not a zero-temperature limit, but a limit of \textbf{maximum persistence}, where any configuration with a non-zero Entropic Action (i.e., any imperfection) is exponentially suppressed.
    \item \textbf{The Ground State:} The ``Gibbs State'' is the unique configuration $S = \{D, \Delta, \nu, \sigma, \chi\}$ that minimizes the Entropic Action, surviving the $\beta \to \infty$ limit with non-zero probability. It is the theoretical ground state of the universe's possible geometries.
\end{itemize}
This framework allows us to treat the selection of our universe's fundamental parameters as a phase transition in the space of all possible mathematical structures.
...
For the gauge group to support chiral matter, the interaction order must match the minimal unifying representation. As established in \cref{sec:system4_derivationd_sytem_logic}, this requires $\sigma = 5$ (the dimension of $SU(5)$).
\begin{equation}
