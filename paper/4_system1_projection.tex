...
Furthermore, this excludes $n=10$ (Superstring Theory) as a standalone geometric substrate. Because $10 \not\equiv 0 \pmod{8}$, a 10-dimensional lattice cannot satisfy \textit{Unitarity} (the requirement for an even, self-dual lattice). To recover these properties, String Theory must introduce auxiliary structures (e.g., Calabi-Yau compactifications or appended internal spaces). Under Informational Energetics, these ad hoc additions require external configuration memory, the selection of which compactification manifold is not determined by the theory's intrinsic constraints, violating the \textbf{Closure Constraint}. The substrate must solve the constraints natively; it cannot borrow structure from outside.
...
However, by the \textbf{Closure Constraint}, the vacuum possesses no \textbf{external configuration memory} to hold this selection bit. If the vacuum geometry is not mathematically unique, it requires an arbitrary initial condition parameter to define its structure. This implies a ``pre-geometric'' memory, explicitly violating the definition of the vacuum as the fundamental ground state.
...
The maximal rank subgroups of $E_8$ include $A_8$, $D_8$, and $D_4 \oplus D_4$\cite{conway_sphere_1988}. The first two fail the self-duality 
preservation requirement:
\begin{itemize}
    \item $A_8$ yields a root lattice with $\det = 9$, requiring scaling factors that violate unimodularity.
    \item $D_8^+$ contains vectors of norm 1 (odd), violating the Evenness requirement established in 
    \cref{sec:kneserstheorem}.
\end{itemize}
Only $D_4 \oplus D_4$ preserves both self-duality and evenness while maintaining the integer norm condition $\mathbb{Z}[\mathbf{v}] \subset \Lambda^*$, making it the unique valid decomposition \cite{conway_sphere_1988}.

This $D_4 \oplus D_4$ decomposition partitions the 8 dimensions into two orthogonal sectors with distinct physical roles:
\begin{enumerate}
...
Because the $Spin(8)$ chiral spinors lack complex structure, and $Spin(9)$ complex spinors lack chirality, the minimal extension that simultaneously supports Complex, Chiral representations (Weyl-Dirac spinors) is $Spin(10)$.
...
For information to be conserved (Unitarity), the evolution of a state must be unambiguous and perfectly reversible. To enforce this, we must examine the algebraic structure of the system's fundamental causal updates.

While the substrate is an 8-dimensional geometric structure, the fundamental state transition, a signal propagating from one node to another, defines a 1-dimensional spatial trajectory evolving over 1-dimensional time. Because the vacuum possesses a fundamental temporal cycle, this sequence of updates must eventually close, forming a periodic $(1+1)D$ causal loop of length $\Delta$. 

For the system to preserve Unitarity, the history of a state traversing this causal loop must be uniquely reconstructable. Mathematically, this requires the algebraic field governing the discrete cycle to be a Unique Factorization Domain (a class number of $h=1$). 

If the field has a class number $h > 1$, multiple distinct causal histories map to the exact same macroscopic state. This manifests as \textbf{Causal Degeneracy}: the vacuum would be unable to map an output state back to its specific input history, violating the unitary requirement that the evolution operator be uniquely invertible ($\hat{U}^\dagger \hat{U} = I$). 

As rigorously derived in \cref{app:algebraic_geometry_causal_loop} via the theory of Complex Multiplication on elliptic curves, this discrete geometric requirement strictly limits the temporal period $\Delta$. The Stark-Heegner theorem \cite{stark_complete_1967} provides the complete list of discriminants $\Delta$ that satisfy this $h=1$ Unitarity constraint:
