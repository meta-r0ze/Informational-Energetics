...
The architectural requirements of System 0 translate into a search for the unique discrete geometric structure satisfying these constraints.
...
\begin{enumerate}
    ...
    \item By the \textbf{Closure Constraint} (\cref{subsec:closure}), the vacuum admits no external configuration memory. The algorithmic specification complexity of the substrate's geometry is therefore \textbf{zero bits} beyond the architectural constraints derived in System 0.
...
\begin{enumerate}
    ...
    To support the \textbf{Causality} component (Arrow of Time), the system must distinguish ``Input'' from ``Output.'' Geometrically, this requires \textbf{Chirality}, the ability to distinguish Left-handed projections from Right-handed projections, preventing information from reflecting symmetrically across the causal loop. 
\end{enumerate}
...
Because spatial position and internal boundary topology are mutually independent, the total defect configuration space factorizes as the Cartesian product $\mathcal{C}_{\text{defect}} = \mathcal{M}_3 \times S^2$. The additivity of their respective dimensions is mandated by the Orthogonality Condition (\cref{sec:torus}), yielding the minimal geometric embedding dimension $\sigma$:
...
Causal propagation on the $E_8$ substrate is restricted to a local $(1+1)\text{D}$ plane. Because the vacuum possesses a fundamental discrete temporal cycle $\Delta$, the state evolution forms a periodic loop. Topologically, this history maps to a real torus $\mathbb{T}^2$; analytically, we model it as a complex torus $\mathbb{C}/\Lambda$ (an elliptic curve) with lattice basis $\{1, z\}$. The Lorentzian signature imparts a complex structure, pairing the temporal step with the spatial step ($\delta t = i\,\delta x$), embedding the generator in the imaginary quadratic field $K = \mathbb{Q}(\sqrt{-\Delta})$.

To preserve the self-duality of the parent $E_8$ lattice ($\Lambda = \Lambda^*$), the partition function on $\mathbb{C}/\Lambda$ must remain invariant under the modular transformation $S: z \to -1/z$. This requires $z$ to lie within the ring of integers $\mathcal{O}_K$ of the field $K$. Because the vacuum admits no external specification memory (Closure Constraint), the causal loop cannot be defined by a sub-order of conductor $f > 1$; such a structure would require additional integer parameters, violating self-definition. The geometric period $\Delta$ must therefore coincide with the fundamental discriminant of the maximal order, yielding the strict identification:
...
\textbf{Failure Mode ($h > 1$):} If $h(D) > 1$, the ideal class group contains non-trivial classes, giving rise to multiple non-isomorphic (though isogenous) elliptic curves sharing the same endomorphism ring. These correspond to topologically distinct causal histories that generate locally identical observable states. An observer at a future node cannot uniquely invert the state transformation, resulting in \textbf{Causal Degeneracy} and a breakdown of unitary evolution. The h(D)=1 condition is therefore both necessary and sufficient under the Closure Constraint.

Consequently, the unitarity requirement strictly restricts $\Delta$ to the Stark--Heegner numbers (the absolute values of the fundamental discriminants of the nine imaginary quadratic fields of class number one):
\begin{equation}
...
\begin{itemize}
    ...
    \item \textbf{Toll: The causal cost of persistence ($\tau = -1$, $M=11$).} Structurally, the Lorentzian metric ($g_{00} = -1$) encodes time by sacrificing one spatial dimension's definiteness. Operationally, the $M = \Delta - N = 11$ overhead intervals in the fundamental cycle provide the temporal slack required for lossless state transitions.
