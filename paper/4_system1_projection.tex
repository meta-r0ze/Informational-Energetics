...
\subsubsection{The Intrinsic Load (\texorpdfstring{$L_{intrinsic} = \LintrinsicVal$}{Lintrinsic}): Internal Specification}
The information required to specify a topological defect's internal state comprises three orthogonal components. The chiral configuration ($\nu=16$) encodes phase and handedness. The interaction embedding ($\sigma=5$) encodes the coordinate degrees of freedom: spatial address ($D_{space}=3$) and boundary coordinates ($D_{boundary}=2$, the continuous parameters required to specify a point on the boundary surface). The topological invariant ($\chi=2$) encodes the global identity of the boundary, the discrete classification that distinguishes the sphere ($\chi=2$) from all other closed surfaces, independent of local coordinates. Together these three orthogonal components set the internal resolution limit: the precision with which the vacuum can distinguish \emph{what} a defect is (internal state) from \emph{where} it is (spatial position). Because these three components are strictly orthogonal information channels (\cref{sec:entropic_balance_sheet}), their minimal description lengths combine additively via direct sum:
\begin{equation}
    L_{intrinsic} = \nu + \sigma + \chi = 16 + 5 + 2 = \mathbf{\LintrinsicVal}
\end{equation}

\subsubsection{The Embedding Load (\texorpdfstring{$L_{embed} = \LembedVal$}{Lembed}): Manifold Address}
The total information to specify a defect's complete configuration: internal state plus spatial address.
\label{sec:lembed}

To satisfy Causality, the substrate cannot only record a static spatial coordinate ($D=4$). A pure configuration-space point possesses no intrinsic trajectory; its subsequent state would be undefined without external boundary conditions, strictly violating the Closure Constraint. To persist as a self-determining entity, a dynamic state must uniquely specify both its coordinate location ($D=4$) and its causal successor configuration ($D=4$). This geometric doubling ($2D = 8$) is the strict minimum required to define a directed causal trajectory generated entirely from internal state data. Because kinematic phase space is orthogonal to internal state identity, this geometric doubling adds directly:

\begin{equation}
    L_{embed} = L_{intrinsic} + 2D = 23 + 8 = \mathbf{\LembedVal}
\end{equation}

\subsubsection{The Substrate Load (\texorpdfstring{$L_{substrate} = \LsubstrateVal$}{Loverhead})}
...
\end{itemize}
\begin{equation}
    L_{substrate} = (R_M) + \nu = (4 \cdot 43) + 16 = \mathbf{\LsubstrateVal}
\end{equation}
