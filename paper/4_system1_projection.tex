...
Having established that the vacuum must be a Finite, Even, Self-Dual, and Causal information processing substrate we now identify the one unique mathematical structure that satisfies these constraints.
...
\[ D \in \{8, 16, 24, \dots\} \]
This strictly eliminates any lattice solution in dimensions $D<8$, including $D=4$ (Standard Relativity). Superstring Theory ($D=10$) also cannot support a self-dual unitarity condition without auxiliary structures.

\subsubsection{The Principle of Algorithmic Determinism}
\label{sec:principleofalgorithmicdeterminism}

To satisfy the \textbf{Map} and \textbf{Unitarity} pillars, the substrate geometry must be intrinsic and self-defining. It cannot depend on arbitrary selection parameters or external boundary conditions.

We constrain the population of even, self-dual lattices permitted by Kneser's Theorem ($D = 8k$) through the lens of \textbf{Algorithmic Complexity}:

\begin{itemize}
    \item \textbf{D=8:} A unique solution exists (The $E_8$ lattice).
    \item \textbf{D=16:} Two distinct non-isomorphic solutions exist ($E_8 \oplus E_8$ and $D_{16}^+$).
    \item \textbf{D=24:} Twenty-four distinct solutions exist (The Niemeier lattices).
\end{itemize}

A vacuum established at $D=16$ would possess a \textbf{Specification Complexity} of $H = \log_2(2) = 1$ bit. This represents the information required to distinguish between the two topological isomers. 
...
While the physical consequence of this bound is the stabilization of the \textbf{Spinor Double Cover}\footnote{
    \textbf{Physical Correlation:} Fermions live on the spinor double cover of the manifold, requiring a $4\pi$ rotation ($\psi \to -\psi \to \psi$) to return to identity. This implies the effective information content is doubled ($2N$).
...
\begin{itemize}
    ...
    \textit{Falsification expanded}: For $\Delta=67$, we would require a surjective map $f: 32 \twoheadrightarrow 35$, which is impossible by the Pigeonhole Principle, at least 3 overhead intervals would lack controllers, creating acausal gaps that violate the Map pillar.
\end{itemize}
...
We have now identify a unique geometric substrate that satisfies architectural requirements (System 0) and fulfills the IE requirements for a substrate with six pillars of persistence.
...
\CatchFileBetweenTags{\InvLIntrinsic}{calculations/constants.tex}{InvLIntrinsic}
\CatchFileBetweenTags{\InvLEmbed}{calculations/constants.tex}{InvLEmbed}
\CatchFileBetweenTags{\InvLSubstrate}{calculations/constants.tex}{InvLSubstrate}

\subsection{The Derived Loads}
...
\subsubsection{The Intrinsic Load (\texorpdfstring{$L_{intrinsic} = \InvLIntrinsic$}{Lintrinsic})}
...
\subsubsection{The Embedding Load (\texorpdfstring{$L_{embed} = \InvLEmbed$}{Lembed})}
The total configuration space, defined as the sum of the internal state ($L_{intrinsic}$) and the manifold address space.

\textbf{The Spinor Double-Cover ($2D=8$):} Since fermions inhabit the $Spin(4)$ double cover of the $D=4$ manifold, each spatial coordinate requires an additional parity bit to distinguish the two sheets of the cover (the sign under $2\pi$ rotation). This necessitates an embedding cost of $2D=8$ to uniquely specify the state in the manifold.
...
The constraints of \textbf{Finiteness}, \textbf{Unitarity}, and \textbf{Causality} do not permit a family of solutions; they converge on a \textbf{unique} geometric structure: the $E_8$ lattice projected onto a causal $D=4$ manifold.

This is not a model with adjustable parameters. It is the only architecture capable of persistent existence. The entire structure is characterized by five derived integers that are geometry invariants:
\begin{equation}
    \mathbb{S} = \{D=4, \Delta=43, \nu=16, \sigma=5, \chi=2\}
\end{equation}

With a unique substrate for other systems to persist on top of, we have now established the foundation of $E_8$-Persistence theory. In \cref{sec:geometric_impedance}, we will use $\mathbb{S}$ as our sole input to derive the fine-structure constant $\alpha^{-1}$, producing the theory's first quantitative prediction for comparison with experiment.