...
The projection $E_8 \to D_4 \oplus D_4$ creates two orthogonal, isomorphic sublattices. As derived in \cref{sec:chiralcapacity}, each $D_4$ factor intrinsically requires a bit-depth of $\nu=16$ to support complex phase and directional flow via paired chiral spinors ($8_L \oplus 8_R$). Because the decomposition is symmetric, both sectors inherit this same 16-channel structure. The total node capacity is therefore the direct sum:
