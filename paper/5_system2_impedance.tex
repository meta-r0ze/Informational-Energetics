...
\begin{enumerate}
    \item \textbf{State Selection ($1/N^3$):} The lattice substrate encodes state transitions as three-point interactions, reflecting the fundamental vertex structure of quantum fields. Each leg of the interaction requires independent selection from the $N=32$ available chiral channels, yielding the $1/N^3$ suppression as the entropic cost of selecting one specific transition from the available configuration space.
...
\begin{itemize}
    \item \textbf{CODATA Average (2022):} \AlphaInvExperimentalValue
    \item \textbf{Precision:} \AlphaInvAccText

    \item \textbf{Morel (2020):} \AlphaInvMorelExperimentalValue
    \item \textbf{Precision:} \AlphaInvMorelAccText    

    \item \textbf{Parker (2018):} \AlphaInvParkerExperimentalValue
    \item \textbf{Precision:} \AlphaInvParkerAccText    
\end{itemize}
...
\begin{tabular}{lll}
\toprule
\textbf{Source} & \textbf{Value ($\alpha^{-1}$)} & \textbf{$\Delta$} \\
\midrule
IE & $\qty{\AlphaInvVal \pm 0.0}{}$ & --- \\
...
\bottomrule
\end{tabular}
\caption{Numerical comparison of the IE theoretical derivation against experimental standards.}
