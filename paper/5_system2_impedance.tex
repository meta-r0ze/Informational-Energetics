...
The manifold resolution $R_M = D\Delta = 172$ is the total available coordination capacity, the maximum distinct spacetime addresses per fundamental cycle. A localized defect consumes exactly $\sigma = 5$ degrees of freedom to define its geometric embedding, leaving residual capacity:
...
As established in the Entropic Balance Sheet (\cref{sec:entropic_balance_sheet}), the six existential pillars operate on geometrically disjoint domains assigned by the $E_8 \to D_4 \oplus D_4$ projection. Capacity and Margin govern metric volume; Map governs topological boundaries; Governor regulates temporal phase density; and Toll dictates combinatorial channel selection. The Protocol mediates the residual coordination capacity between the orthogonal sectors. Non-linear cross-terms are structurally forbidden as the projection shares no geometric generators across sectors.

In standard statistical mechanics, independent degrees of freedom yield a strictly factorized partition function ($\mathcal{Z}_{total} = \prod \mathcal{Z}_i$). Because the entropic action is proportional to the negative logarithm of the state sum, this exact factorization mathematically mandates a strictly linear, additive entropic action at lowest order. The distinct functional form of each term, whether a boundary measure, an inverse capacity, or a phase space ratio is the strict, dimensionally-appropriate information-theoretic measure for that specific, orthogonal geometric domain.

The linear combination is therefore the rigorous physical consequence of substrate orthogonality. Two constraints rigidly bound this summation, ensuring it is a complete and minimal basis:
...
Rather than treating charge as a postulated particle property, its fundamental magnitude is strictly bounded by this geometric limit. To compare this substrate bound with macroscopic measurement, we apply the standard dimensional isomorphism. The dimensionless geometric impedance $Z_{geo}$ maps to the inverse fine-structure coupling; in SI units this yields:
