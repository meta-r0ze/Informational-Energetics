...
\begin{equation}
\mathcal{Z}_{\text{total}} = \mathcal{Z}_{\text{CAP}}\,\mathcal{Z}_{\text{MAP}}\,\mathcal{Z}_{\text{PRO}}\,\mathcal{Z}_{\text{GOV}}\,\mathcal{Z}_{\text{TOL}}\,\mathcal{Z}_{\text{MAR}},
\end{equation}
because the $E_8 \to D_4 \oplus D_4$ projection assigns each pillar to a geometrically disjoint domain. 

As established in the Entropic Balance Sheet (\Cref{sec:entropic_balance_sheet}), at strict zero flux these domains share no generators, so their mutual information vanishes and cross-terms are forbidden. The geometric impedance is therefore the additive entropy. Because Capacity and Map enter as exact Boltzmann weights while the remaining pillars enter as small multiplicative modifiers $(1 \pm P_i)$, their entropic contributions evaluate via $-\ln(1 \pm P_i) \approx \mp P_i$:

\begin{equation}
    Z_{\text{geo}} = -\ln \mathcal{Z}_{\text{total}} = \sum_{i=1}^6 (-\ln \mathcal{Z}_i) \equiv \sum_{i=1}^6 Z_i
\end{equation}
