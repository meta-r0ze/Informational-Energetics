...
As established in the Entropic Balance Sheet (\Cref{sec:entropic_balance_sheet}), the six existential pillars operate on geometrically disjoint domains assigned by the $E_8 \to D_4 \oplus D_4$ projection. Capacity and Margin govern metric volume; Map governs topological boundaries; Governor regulates temporal phase density; and Toll dictates combinatorial channel selection. The Protocol mediates the residual coordination capacity between the orthogonal sectors. Because these domains share no geometric generators, their mutual information vanishes identically at the strict zero-flux baseline. While dynamical excitations inevitably generate non-linear cross-terms, Quiescent Equilibrium represents the static structural floor where no active flux exists to mediate these couplings.

Crucially, the state sum separates into the macroscopic structural baseline (the exact geometric invariants of Capacity and Map) and multiplicative modifiers $(1 \pm P_i)$ for Protocol, Governor, Toll, and Margin, representing fractional phase-space modifications to that baseline. In Quiescent Equilibrium these fractional weights are small ($P_i \ll 1$), so their entropic contributions evaluate via the first-order limit $-\ln(1 \pm P_i) \approx \mp P_i$, while the macroscopic baseline terms enter exactly:

\begin{equation}
\begin{split}
    Z_{geo} &\propto -\ln \mathcal{Z}_{total} = \sum_{i=1}^6 (-\ln \mathcal{Z}_i) \equiv \sum_{i=1}^6 Z_i
\end{split}
\end{equation}

The distinct functional form of each term—whether a macroscopic boundary measure ($\pi\Delta$), an exact topological invariant ($\chi$), or a perturbative phase-space probability ratio ($P \ll 1$)—is therefore the rigorously appropriate, dimensionless information-theoretic measure for that specific, orthogonal geometric domain, seamlessly unified at the linear limit.
