...
\end{equation}
Our parameter-free calculation yields $\alpha^{-1} = \AlphaInvVal \dots$, a value in agreement with the CODATA consensus to within 0.6$\sigma$.
%This result is further validated by an independent numerical simulation of the $E_8$ substrate, which dynamically recovers the constant to within $0.0007\%$ precision, confirming that it is an emergent property of the lattice geometry, not an externally fitted parameter.
